...
# scripts). setup.sh owns src/modules/ as a whole, including the generated Modules.h and
# .gitignore, plus build/modules.lock and build/toolchain/. Re-running replaces them.
...
die()  { echo "❌ $*" >&2; exit 1; }
info() { echo "➜ $*"; }
usage() { sed -n '4,34p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'; exit "${1:-0}"; }
...
mkdir -p "${staged_modules}" "${BUILD_DIR}"
...
# Generated by scripts/setup.sh: src/modules/ is installed, not part of this repository.
*
EOF
