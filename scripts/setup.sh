...
# setup.sh — assemble XeWe OS: choose modules, install them into src/, install the build
# toolchain into build/toolchain, and run the build environment setup.
#
# Usage:
#   scripts/setup.sh                            interactive: pick modules in a checklist
#   scripts/setup.sh --modules wifi,scheduler   non-interactive (required modules are added)
...
#   --skip-build-setup          don't run the toolchain's setup_build_environment
#   --force                     replace src/<Folder> directories that setup.sh didn't install
...
#
# Modules are copied as src/<Folder>/ only (no .git, no module .ino or scripts). setup.sh owns
# what it installs: src/Modules.h, src/.gitignore, the module folders recorded in
# build/modules.lock, and build/toolchain/. Re-running replaces them.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC_DIR="${PROJECT_ROOT}/src"
BUILD_DIR="${PROJECT_ROOT}/build"
LOCK_FILE="${BUILD_DIR}/modules.lock"
MODULES_HEADER="${SRC_DIR}/Modules.h"
...
die()  { echo "❌ $*" >&2; exit 1; }
info() { echo "➜ $*"; }
usage() { sed -n '4,35p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'; exit "${1:-0}"; }
...
# ---------------------------------------------------------------------------
# fetch the toolchain first, so a failed download leaves src/ untouched
# ---------------------------------------------------------------------------
if [[ ${skip_toolchain} -eq 0 ]]; then
  staged="${WORK}/toolchain"
  if [[ -n "${toolchain_source}" ]]; then
    [[ -f "${toolchain_source}/toolchain.sh" && -d "${toolchain_source}/scripts" ]] \
...
# ---------------------------------------------------------------------------
# install modules into src/
# ---------------------------------------------------------------------------
mkdir -p "${SRC_DIR}" "${BUILD_DIR}"

old_folders=""
if [[ -f "${LOCK_FILE}" ]]; then
  old_folders="$(grep -v '^#' "${LOCK_FILE}" | cut -d'|' -f2 | tr '\n' ' ')"
fi
...
for slug in ${ORDER}; do
  ...
  [[ -n "${folder}" && -d "${dir}/src/${folder}" ]] || die "${MODULE_PREFIX}${slug}: src/${folder} not found"
  [[ "${folder}" != *"/"* && "${folder}" != "."* ]] || die "${MODULE_PREFIX}${slug}: invalid folder '${folder}'"
  case " ${old_folders} " in
    *" ${folder} "*) ;;
    *) if [[ -e "${SRC_DIR}/${folder}" && ${force} -eq 0 ]]; then
         die "src/${folder} exists and was not installed by setup.sh; move it away or pass --force"
       fi ;;
  esac
  new_folders="${new_folders} ${folder}"
done

# remove modules installed earlier that are no longer selected
for folder in ${old_folders}; do
  case " ${new_folders} " in *" ${folder} "*) ;; *)
    [[ -n "${folder}" && "${folder}" != *"/"* ]] && rm -rf "${SRC_DIR:?}/${folder}" && info "removed src/${folder}" ;;
  esac
done

lock="# generated by scripts/setup.sh: slug|folder|source|ref|commit"
includes=""
declarations=""
for slug in ${ORDER}; do
  dir="$(module_repo "${slug}")"
  ...
  cp -R "${dir}/src/${folder}" "${SRC_DIR}/${folder}"
  rm -rf "${SRC_DIR}/${folder}/.git"
...
"
  info "installed src/${folder}  (${MODULE_PREFIX}${slug} $(prop "${manifest}" version))"
done

printf '%s\n' "${lock}" > "${LOCK_FILE}"

cat > "${MODULES_HEADER}" <<EOF
// Generated by scripts/setup.sh; do not edit. Re-run setup.sh to change modules.
...
{
  echo "# Generated by scripts/setup.sh: installed modules are not part of this repository."
  echo "/.gitignore"
  echo "/Modules.h"
  for folder in ${new_folders}; do echo "/${folder}/"; done
} > "${SRC_DIR}/.gitignore"

# ---------------------------------------------------------------------------
# toolchain into build/toolchain
# ---------------------------------------------------------------------------
if [[ ${skip_toolchain} -eq 0 ]]; then
  rm -rf "${BUILD_DIR:?}/toolchain"
  mv "${staged}" "${BUILD_DIR}/toolchain"
  chmod +x "${BUILD_DIR}/toolchain/toolchain.sh" "${BUILD_DIR}/toolchain/scripts/"*/*.sh 2>/dev/null || true
  info "installed build/toolchain  (${toolchain_desc})"
fi
...
if [[ ${skip_build_setup} -eq 1 ]]; then
  echo "Build setup skipped; run it later with build/toolchain/scripts/<mac|linux>/setup_build_environment.sh"
  exit 0
fi
[[ -d "${BUILD_DIR}/toolchain" ]] || die "build/toolchain is missing; run without --skip-toolchain"

case "$(uname -s)" in
  Darwin) platform=mac ;;
  Linux)  platform=linux ;;
  *) echo "Run build\\toolchain\\scripts\\windows\\setup_build_environment.ps1 in PowerShell to finish."
     exit 0 ;;
esac
info "running build/toolchain/scripts/${platform}/setup_build_environment.sh"
exec "${BUILD_DIR}/toolchain/scripts/${platform}/setup_build_environment.sh"
